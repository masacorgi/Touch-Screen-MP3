...
volatile unsigned char rxck1,rxck2,rxck3,rx_led,tx_led,this_id,tx1_busy,tx2_busy,tx3_busy;
...
void uasrt_send (unsigned char ch,char *buff,int length)
{
      switch (ch) {
      	 case 1:
           while (tx1_busy);
...
void s_printf (unsigned char ch,char *form,...)
{
      va_list argptr;
      va_start(argptr,form);
      vsprintf(sbuff,form,argptr);
      uasrt_send(ch,sbuff,strlen(sbuff));
      va_end(argptr);
}
