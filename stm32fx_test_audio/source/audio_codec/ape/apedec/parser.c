...
int ape_parseheader(FIL* fx, struct ape_ctx_t* ape_ctx)
{
    ...
    f_read(fx,(unsigned char *)&ape_ctx->magic,4,(UINT *)&n);
    if (n != 4) return -1;
    ...
    if (ape_ctx->fileversion >= 3980)
    {
        ...
        if (read_uint32(fx,&ape_ctx->wavtaillength) < 0)
            return -1;
		f_read(fx,(unsigned char *)&ape_ctx->md5,16,(UINT *)&n);
        if ( n!= 16)
            return -1;
    ...
    {
        ape_ctx->seektable = (void *)malloc(ape_ctx->seektablelength);
